...
namespace WinPE
{
    ...
    public class PeHeaderReader
    {
        ...
        public PeHeaderReader(int processId)
        {
            // Read
            ProcessMemoryManager pmm = new ProcessMemoryManager(processId);
            pmm.ReadMemory(0, new IntPtr(0x400000), out byte[] data, 1024, out int bytesRead);
            ...
            using (MemoryStream ms = new MemoryStream(data))
            {
                using (BinaryReader br = new BinaryReader(ms))
                {
                    ...
                    for (int headerNo = 0; headerNo < imageSectionHeaders.Length; ++headerNo)
                    {
                        imageSectionHeaders[headerNo] = FromBinaryReader<IMAGE_SECTION_HEADER>(br);
                    }
                }
            }
        }
